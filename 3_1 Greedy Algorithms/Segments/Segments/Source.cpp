...
int main(void) {
	...
	while (k != 0)
	{
		for (i = 0; i < n; i++) {
			...
			//для всех отрезков кроме последнего, так как его не с чем сравнивать (иначе [i+1] выйдет за границы массива)
			if (i != (n - 1)) {
				...
				if (segments[i][1] >= segments[i + 1][0]) {
					...
					if (segments[i][0] < segments[i + 1][0]) { segments[i][0] = segments[i + 1][0]; }
					if (segments[i][1] > segments[i + 1][1]) { segments[i][1] = segments[i + 1][1]; }
					if (segments[i][1] == segments[i + 1][0]) { segments[i][0] = segments[i + 1][0]; }
					//если "склеили" последний отрезок (i=n-1) с предпоследним, то "укорачиваем" массив на 1
					if (i == (n - 2)) {
						n--;
					}
